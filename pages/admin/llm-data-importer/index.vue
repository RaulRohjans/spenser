...
<template>
    <UForm :state="state" class="space-y-4" @submit="onSave">
        <UFormField :label="$t('LLM Provider')" name="provider" class="w-full" :error="error">
            <USelect :key="providerSelectKey" v-model="state.provider" :items="getProviderOptions" />
        </UFormField>

        <template v-if="state.provider === 'gpt'">
            <UFormField :label="$t('GPT Model')" name="gptModel" class="w-full" :error="!!error">
                <UInput v-model="state.gptModel" />
            </UFormField>

            <UFormField :label="$t('GPT Token')" name="gptToken" class="makeit-static" :error="!!error">
                <UInput v-model="state.gptToken" type="password" />
            </UFormField>
        </template>

        <template v-else-if="state.provider === 'ollama'">
            <UFormField :label="$t('Ollama Model')" name="ollamaModel" class="w-full" :error="!!error">
                <UInput v-model="state.ollamaModel" />
            </UFormField>

            <UFormField :label="$t('Ollama URL')" name="ollamaUrl" class="w-full" :error="!!error">
