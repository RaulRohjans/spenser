<script setup lang="ts" generic="T">
    ...
    const columnOptions = computed(
        () =>
            props.tableApi
                ?.getAllColumns()
                .filter((c) => c.columnDef.meta?.searchable !== false) // If nothing is specified, default is true
                .map((c) => ({
    ...
    watchEffect(() => {
        ...
        if (
            ...
            !validValues.includes(column.value) &&
            validValues[0]
        )
    ...
    ) // Add debouce to limit query rate
</script>

<template>
    <div v-if="tableApi" class="flex flex-col sm:flex-row gap-1">
        <USelect v-model="column" class="min-w-26" :items="columnOptions" />
        <UInput
            v-model="search"
