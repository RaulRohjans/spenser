...
export default defineEventHandler(async (event) => {
    ...
    const query = await db.selectFrom('budget')
        ...
        .leftJoin('transaction',(join) => join
        ...
        .select(({fn}) => [
            fn.sum(sql<number>`case when "transaction"."value" < 0 then "transaction"."value" * -1 when "transaction"."value" >= 0 then 0 end`).as('expenses')
        ])
