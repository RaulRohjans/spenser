...
export default defineEventHandler(async (event) => {
    ...
    const query = db.selectFrom('currency')
        .selectAll()

        // Search Filter
        .$if(true, qb => applySearchFilter(qb, search?.toString(), searchColumn?.toString() || 'currency.symbol'))
    ...
    const totalRecordsRes = await db.selectFrom('currency')
        .select(({ fn }) => [
