...
export default defineEventHandler(async (event) => {
    ...
    if(!username || !password)
        throw createError({ statusCode: 403, statusMessage: 'Empty login fields' })    
    console.log(users)
...
const validateLoginCredentials = function(username: string, password: string) {
    ...
    if(users.length == 0) {
        if(username != 'admin' || password != 'admin') 
            throw createError({ statusCode: 403, statusMessage: 'Invalid login credentials' })
...
const firstLogin = function() {
    ...
    const user: User = { // Create User model
        username: 'admin',
